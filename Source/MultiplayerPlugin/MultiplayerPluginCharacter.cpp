...
void AMultiplayerPluginCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		EnhancedInputComponent->BindAction(MouseLookAction, ETriggerEvent::Triggered, this, &AMultiplayerPluginCharacter::Look);
	...
	{
		UE_LOG(LogMultiplayerPlugin, Error, TEXT("'%s' Failed to find an Enhanced Input component! This template is built to use the Enhanced Input system. If you intend to use the legacy system, then you will need to update this C++ file."), *GetNameSafe(this));
	}
}
...
void AMultiplayerPluginCharacter::DoMove(float Right, float Forward)
{
...
void AMultiplayerPluginCharacter::DoLook(float Yaw, float Pitch)
{
	if (GetController() != nullptr)
	{
...
void AMultiplayerPluginCharacter::DoJumpStart()
{
	// signal the character to jump
	Jump();
}

void AMultiplayerPluginCharacter::DoJumpEnd()
{
